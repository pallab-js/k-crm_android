...
object DatabaseModule {
    ...
    fun provideNexusDatabase(
        ...
        val passphrase = "nexus_crm_secure_passphrase".toByteArray()
        ...
        .addCallback(object : RoomDatabase.Callback() {
            override fun onCreate(db: SupportSQLiteDatabase) {
                super.onCreate(db)
                // We'll seed data after the database is built
            }
        }).build()
        
        // Seed data if database is empty (simple check)
    ...
    private suspend fun seedData(db: NexusDatabase) {
        if (db.contactDao().getContactCountSync() > 0) return
